...
// Public profile page. Route: /profile/:username
// Shows user info, stats, and their public library.
    ...
    <div>
      ...
      <div className="section-block">
        <div className="section-header">
          <span>{t.profile}</span>
          {isOwnProfile && (
            <span style={{ fontSize: 12, cursor: 'pointer' }} onClick={() => navigate('/settings')}>
              ⚙ Edit
            </span>
          )}
        </div>
        ...
        <div className="section-body">
          ...
          {entries.map(entry => {
            ...
            return (
              ...
              >
                {media.cover_url
                  ? <img className="library-row-cover" src={media.cover_url} alt={media.title} />
                  : <div className="library-row-cover" style={{
                      background: 'var(--bg-secondary)', display: 'flex',
                      alignItems: 'center', justifyContent: 'center',
                      fontSize: 10, color: 'var(--text-muted)', padding: 4, textAlign: 'center' }}>
                      {media.title}
                    </div>
                }
                <div className="library-row-info">
                  ...
                  <div className="library-row-meta">
                    ...
                    <span style={{ marginLeft: 6, fontSize: 10, color: 'var(--text-muted)',
                      textTransform: 'uppercase' }}>{media.type}</span>
                  </div>
                </div>
                {entry.rating && (
                  <div style={{ flexShrink: 0, fontWeight: 700, color: 'var(--primary)' }}>
                    ★ {entry.rating}
                  </div>
                )}
              </div>
